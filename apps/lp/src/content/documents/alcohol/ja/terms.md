...
この利用規約（以下、「本規約」といいます。）は、禁酒チャレンジ（以下、「運営」といいます。）がこのアプリ上で提供するサービス（以下、「本サービス」といいます。）の利用条件を定めるものです。登録ユーザーの皆さま（以下、「ユーザー」といいます。）には、本規約に従って、本サービスをご利用いただきます。
...
## 第4条（禁止事項）
運営は、不適切なコンテンツおよび迷惑行為を行うユーザーに対してゼロトレランス（一切許容しない）ポリシーを採用します。本条に違反するコンテンツは速やかに削除し、違反行為を行ったユーザーのアカウントは事前の通知なく停止または削除します。
...
## 第4条の2（子どもの安全に関するポリシー / CSAE・CSAM の禁止）
禁酒チャレンジ（以下、「本サービス」）は、児童の性的虐待や搾取（CSAE／CSAM）に該当する一切の行為およびコンテンツを明確に禁止します。

ユーザーは、以下の行為を行ってはなりません。

1. CSAE／CSAM に該当する画像、動画、音声、テキスト、リンク等の投稿・共有・送信
2. 未成年に対する性的な発言、誘引、脅迫、要求
3. 未成年者との性的な目的を持った連絡・コミュニケーション
4. 上記に関連する勧誘、仲介、共犯行為

運営は、これらの禁止行為が確認された場合、以下の措置を講じます。

- 問題コンテンツの即時削除
- アカウントの停止または利用契約の解除
- 必要に応じて関係当局への通報

運営は、Google Play 子どもの安全基準ポリシーおよび関連法令を遵守し、本サービスを安全に運営します。
